...
{
    public class ElementosDetalleNovedadPdfReport
    {
         public static byte[] CreateInMemoryPdfReport(string wwwroot,List<ElementoReportViewModel> ListElementos)
        {
			
            return CreateHtmlHeaderPdfReport(wwwroot,ListElementos,"","",0).GenerateAsByteArray(); // creating an in-memory PDF file
        }
       
		public static IPdfReportData CreateHtmlHeaderPdfReportStream(string wwwroot, Stream stream, List<ElementoReportViewModel> reportElementos,string date_start_report,string date_end_report,long detalle_tipo_novedad_id)
        {
               return CreateHtmlHeaderPdfReport(wwwroot,reportElementos, date_start_report, date_end_report,detalle_tipo_novedad_id).Generate(data => data.AsPdfStream(stream, closeStream: false));
        }

        public  static PdfReport CreateHtmlHeaderPdfReport(String wwwroot,List<ElementoReportViewModel> reportElementos,string date_start_report,string date_end_report,long detalle_tipo_novedad_id)
		{
			return new PdfReport().DocumentPreferences(doc =>
			{
			 ...
			 .PagesHeader(header =>
			 {
				 header.HtmlHeader(rptHeader =>
				 {
                     ...
                     rptHeader.AddGroupHeader(groupHeader =>
					 {
						 ...
						 ///var nombre_detalle_tipo_novedad= data.GetSafeStringValueOf<ElementoReportViewModel>(x => x.Nombre_Detalle_Tipo_Novedad);
						 var id_ciudad= long.Parse(data.GetSafeStringValueOf<ElementoReportViewModel>(x => x.Ciudad_Id));

						
						 long novedades_total=reportElementos.Where(a=>a.Elemento_Id==long.Parse(elemento_id)).Count();
						...
						var resumen_poste= string.Format(@"<table style='width: 100%;font-size:7pt;font-family:tahoma;'>
																		...
																		<tr>
																			<th ><strong>Longitud Poste:</strong></th>
																			<td align='center'>{2}</td>
																			<th ><strong>Detalle Novedad:</strong></th>
																			<td align='center'>{3}</td>
																		</tr>
																		</table>",ciudad,detalle_elemento.Nombre_Tipo_Novedad,detalle_elemento.Longitud,detalle_elemento.Nombre_Detalle_Tipo_Novedad);
																		...
																		var posteHeader=string.Format(@"
																			...
																			</table>",resumen_poste,detalle_elemento.Elemento_Id,detalle_elemento.Nombre_Detalle_Tipo_Novedad,detalle_elemento.Longitud,detalle_elemento.Nombre_Estado,detalle_elemento.Valor_Nivel_Tension,detalle_elemento.AlturaDisponible,detalle_elemento.ResistenciaMecanica,detalle_elemento.Nombre_Material,detalle_elemento.Retenidas,detalle_elemento.Direccion_Elemento,detalle_elemento.Coordenadas_Elemento);

                                                        var listfotos= new List<FotoViewModel>();
														 listfotos=detalle_elemento.Fotos;
														...
														if(countfotos>4){
																foreach(var foto in listfotos){
																	...
																	if(i==1){
																		...
																		tablafotos += string.Format(@"<td><img  width='120'  src='http://54.86.105.4{0}' /><p>Titulo: {1}</p><p>Descripcion: {2}</p></td>", foto.Ruta,foto.Titulo,foto.Descripcion );
																	}else if(i==2){
																		tablafotos += string.Format(@"<td><img  width='120'  src='http://54.86.105.4{0}' /><p>Titulo: {1}</p><p>Descripcion: {2}</p></td>", foto.Ruta,foto.Titulo,foto.Descripcion );
																	}else if(i==3){
																		tablafotos += string.Format(@"<td><img  width='120'  src='http://54.86.105.4{0}' /><p>Titulo: {1}</p><p>Descripcion: {2}</p></td>", foto.Ruta,foto.Titulo,foto.Descripcion );
			 ...
			 .MainTableDataSource(dataSource =>
			 {
				 ...
				 listOfRows = listOfRows.OrderBy(x => x.Ciudad).ThenBy(x => x.Nombre_Nivel_Tension).ToList();
