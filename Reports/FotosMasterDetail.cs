...
{
    public class FotosMasterDetail
    {
     ...
     /*  public MasterDetailsPdfReport(IElementoRepository  IelementosRepository){
        ...
        public static byte[] CreateInMemoryPdfReport(string wwwroot, List<FotoViewModel> Listfotos)
        {
            return CreateMasterDetailsPdfReport(wwwroot, Listfotos).GenerateAsByteArray(); // creating an in-memory PDF file
        }
       
        
         public static IPdfReportData CreateStreamingPdfReport(string wwwroot, Stream stream, List<FotoViewModel> listfotos)
        {
        ...
        public static PdfReport CreateMasterDetailsPdfReport(String wwwroot, List<FotoViewModel> listfotos)
        {
                ...
                .DefaultFonts(fonts =>
            {
                ...
                .PagesHeader(header =>
                {
                    ...
                    header.CustomHeader(new MasterDetailsHeadersFoto { PdfRptFont = header.PdfFont });
                ...
                .MainTableColumns(columns =>
                {
                    ...
                    columns.AddColumn(column =>
                    {
                        column.PropertyName<Foto>(x => x.Novedad_Id);
                        column.CellsHorizontalAlignment(HorizontalAlignment.Center);
                        column.IsVisible(true);
                        column.Order(1);
                        column.Width(2);
                    ...
                    columns.AddColumn(column =>
                    {
                        column.PropertyName<Foto>(x => x.Titulo);
                        column.CellsHorizontalAlignment(HorizontalAlignment.Center);
                        column.IsVisible(true);
                        column.Order(2);
                        column.Width(3);
                    ...
                    columns.AddColumn(column =>
                    {
                        column.PropertyName<Foto>(x => x.Descripcion);
                        column.CellsHorizontalAlignment(HorizontalAlignment.Center);
                        column.IsVisible(true);
                        column.Order(3);
                        column.Width(3);
                    ...
                    columns.AddColumn(column =>
                    {
                        column.PropertyName<Foto>(x => x.Elemento_Id);
                        column.CellsHorizontalAlignment(HorizontalAlignment.Center);
                        column.IsVisible(true);
                        column.Order(4);
                        column.Width(2);
                        column.HeaderCell("Elemento Id");
...
public class CustomFooter : IPageFooter
    {
        ...
        public class MasterDetailsHeadersFoto : IPageHeader
        {
        ...
        {
            var fechacreacion = newGroupInfo.GetSafeStringValueOf<Foto>(x => x.FechaCreacion);
            var codigoapoyo = newGroupInfo.GetSafeStringValueOf<Foto>(x => x.Elemento.CodigoApoyo);
            var altura = newGroupInfo.GetSafeStringValueOf<Foto>(x => x.Elemento.AlturaDisponible);
